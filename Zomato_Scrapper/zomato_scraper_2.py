...
class Zomato(object):
    ...
    def get_menu(self):
        ...
        def get_pic_url(x): return x.find('div', {'id': 'menu-image'}).find('img').get('src')
        try:
    ...
    def get_cost(self):
        ...
        price_value = [int(i.strip()) for i in itemprop.contents[0].contents if type(
            i).__name__ == 'NavigableString'][0]
...
if __name__ == '__main__':
    urls = [
        'https://www.zomato.com/kansas-city/swagat-fine-indian-cuisine-kansas-city'
    ]
    for url in urls:
        z = Zomato(url)
        #print z.get_basics()
